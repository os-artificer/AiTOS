...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf)
{
    ...
    while (block_idx < 140)
    {
        ...
        if (all_blocks[block_idx] == 0)
        {
            ...
            {                                         // not allocate first level indirect block table (12 is mean the 0 th indirect block)
                ...
                block_lba = block_bitmap_alloc(cur_part);
                if (block_lba == -1)
                { // allocate failed , restore before data
...
int32_t dir_remove(struct dir* parent_dir, struct dir* child_dir)
{
    ...
    while (block_idx < 13)
    { // only have a block at sector[0] , other must 0
