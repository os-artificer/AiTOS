...
| `grub-mkrescue`、`xorriso` | GRUB 路径：`make grub-iso`（`install_devenv.sh` 会安装）   |
...
| `GRUB_BOOT`             | `iso`                        | GRUB 介质：`iso`（`bin/aitos.iso`）或 `disk`（`bin/aitos-grub.img`） |
| `QEMU_BOOT`             | `c`（GRUB）/ 默认 `c`（legacy）   | QEMU `-boot order=`；GRUB hybrid ISO 须为 `c`（`-hda` 启动）   |
...
项目提供 **两条并存** 的 x86-64 启动路径，共用同一份 `build/kernel.elf`：

| 路径 | 介质 | 典型命令 | 说明 |
| ---- | ---- | -------- | ---- |
| **Legacy** | `bin/hd60M.img`（自研 MBR → loader） | `make run-qemu` | 日常对照、稳定可用 |
| **GRUB** | `bin/aitos.iso` 或 `bin/aitos-grub.img` | `make run-qemu-grub` | Multiboot2；Phase 1 仅无头串口 |

设计细节见 [docs/design/grub-boot.md](docs/design/grub-boot.md)。

### Legacy 路径

```bash
./scripts/install_devenv.sh   # 首次：依赖 + env.rc

make image                    # 编译并写入 hd60M.img / hd80M.img
make run-qemu                 # 无头：当前终端即 Shell 串口
# 或
make run-qemu-gui             # 本地 GUI：在 QEMU 窗口输入
```

调试（GDB 在 `kmain` 处 pending 断点，输入 `c` 后继续）：

```bash
make debug                    # 无头
make debug-gui                # GUI
make debug-tmux               # tmux 分屏（串口 pty + GDB + 日志）
make stop                     # 停止调试实例（另一终端）
```

### GRUB 路径

额外依赖（`install_devenv.sh` 会安装）：`grub-pc-bin`、`grub-common`、`xorriso`。

```bash
make grub-iso                 # 编译内核并生成 bin/aitos.iso
make run-qemu-grub            # GRUB 菜单约 3s 后自动启动 AiTOS
```

GRUB 调试（先起 QEMU 调试桩，再进 GDB；GRUB 引导完成后停在 `kmain`）：

```bash
make debug-grub               # 一键：debug-qemu-grub + GDB（GRUB_BOOT=iso）
# 在 (gdb) 输入 c，等待 GRUB 倒计时结束并加载内核
make stop                     # 退出 GDB 后也会自动清理；或另开终端执行
```

Phase 2 分区盘（需 root 写盘）：

```bash
make grub-disk                # 生成 bin/aitos-grub.img
make run-qemu-grub-disk       # GRUB_BOOT=disk
make debug-grub-disk          # 同上，调试入口
```

**GRUB 环境变量示例：**

```bash
GRUB_BOOT=disk make run-qemu-grub    # 从 MBR 分区盘启动（需先 make grub-disk）
GRUB_BOOT=iso  make debug-grub       # 默认 ISO
```

**注意：**

- Hybrid ISO 必须以 **`-drive file=...,format=raw` 挂为 `-hda`** 启动（脚本已处理；勿用 `-cdrom`）。
- GRUB Phase 1 **仅无头**（`grub.cfg` 走串口）；`QEMU_GUI=1` 会被 GRUB 脚本忽略。
- 内核链接地址 **`0x100000`**；legacy loader 先加载到 `0x70000` 再拷贝至运行地址。
- 若 `make run-qemu-grub` 串口无输出，可用 `make debug-grub` + GDB 验证 `_start` / `kmain`；启动日志见 `bin/boot-debugcon.log`、`bin/qemu-serial.log`。
...
**GRUB + Multiboot2**（与 legacy 并存，用法见上文 [使用方法](#使用方法) → GRUB 路径）：

```bash
make grub-iso && make run-qemu-grub
make debug-grub
```
...
| 命令                                  | 模式    | 说明                                           |
| ----------------------------------- | ----- | -------------------------------------------- |
| `make run-qemu-grub`                | GRUB  | ISO：`grub-iso` 后无头启动（`-hda` hybrid 镜像）       |
| `make run-qemu-grub-disk`           | GRUB  | 分区盘启动（需 `make grub-disk`）                    |
| `make debug-grub`                   | GRUB  | 一键 GDB（ISO，`pending break kmain`）            |
| `make debug-grub-disk`              | GRUB  | 同上，分区盘介质                                   |
| `make debug-qemu-grub`              | GRUB  | 仅启动 GRUB QEMU 调试桩（不自动进 GDB）                |
| `make grub-iso`                     | —     | 构建 `bin/aitos.iso`                          |
| `make grub-disk`                    | —     | 构建 `bin/aitos-grub.img`（需 root）              |
...
| GRUB + GDB      | `make debug-grub` → GDB 输入 `c`（等 GRUB 自动选单启动）      | GRUB Phase 1 无 GUI              |
...
3. 选择启动配置后按 F5：
   - **Debug AiTOS (QEMU + GDB)** — legacy 路径
   - **Debug AiTOS (GRUB + QEMU + GDB)** — GRUB ISO 路径
...
| 命令                    | 说明                            |
| --------------------- | ----------------------------- |
| `make` / `make image` | 编译并写入 legacy 磁盘镜像             |
| `make grub-iso`       | 构建 GRUB hybrid ISO              |
| `make grub-disk`      | 构建 GRUB MBR 分区盘（需 root）        |
...
arch/x86_64/
  ...
  multiboot2.asm Multiboot2 header（GRUB）
  kernel.asm     入口、中断 stub
  print.asm      底层打印
  linker.ld      链接脚本（内核 @ 0x100000）
boot/            early.c、multiboot2.c（引导信息抽象）
grub/            grub.cfg（GRUB 串口菜单）
...
bin/             磁盘镜像（hd60M.img、hd80M.img、aitos.iso 等）
scripts/         环境安装、QEMU/GRUB 运行与调试（run-qemu-grub.sh、debug-grub.sh 等）
...
**`make run-qemu-grub` 只有 GRUB 菜单、无内核输出**  
确认已 `make grub-iso` 且 ISO 为最新；查看 `bin/boot-debugcon.log`。用 `make debug-grub`，在 GDB 输入 `c` 后检查是否命中 `kmain`。GRUB 路径 Phase 1 仅串口，勿设 `QEMU_GUI=1`。

**`make grub-disk` 失败**  
需要 root 权限挂载 loop 设备；仅在需要验证 MBR 分区盘时使用，日常开发用 `make grub-iso` 即可。