...
// kernel thread entry: run interactive shell (must not use process_execute with kernel code)
static void init_kthread(void* arg UNUSED)
{
    shell_run();
}
...
// print buffer filled with space
...
void thread_init(void)
{
    ...
    lock_init(&pid_lock);
    // create init kernel thread that runs the shell
    thread_start("init", default_prio, init_kthread, NULL);
